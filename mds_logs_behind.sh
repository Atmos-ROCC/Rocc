...
find /var/tmp/logsbehind/ -ctime +2 -exec rm -f  {} \;
[[ ${allmds[@]} == ${initmds[@]} ]] && { rm -rf /var/tmp/logsbehind; }
...
for port in $allmds; do
  ...
  if [[ $logsbehind != 0 && $logsbehind != "404_hes-dead-jim" ]]; then
    ...
    [[ $(echo $logchange|sed -r "s/\x1B\[([0-9]{1,2}(;[0-9]{1,2})?)?[m|K]//g") -ne 0 ]] && { [[ $(echo $eta|awk -F '.' '{print $1}') -ge 60 ]] && { hours=$(echo "$eta/60"|bc); minutes=$(echo "$eta-($hours*60)"|bc|awk -F '.' '{print $1}'); eta=$(echo ${hours}:${minutes}); }; }
    ...
    printf "Port  : %-6s Master Host-Port: %17s:%-7s Master Log HLSN: %-11s\nChange: %-6s LogsBehind: %-6s ETA: %-6s Local Log HLSN: %-11s\n" $(echo $port $masterhost $masterport $masterhlsn $logchange $logsbehind $eta $localhlsn)
  elif [[ $logsbehind = "404_hes-dead-jim" ]]; then echo -e $red"Skipping $port, not able to gather lsn(s) $logsbehind \nMaster Log HLSN: $masterhlsn Local Log HLSN: $localhlsn"$normal
  else initmds+=" $port"
  fi
done
...
while getopts "aup:hv" options;do
  case $options in
    ...
    ;;
    u) allmds=$(mauisvcmgr -s mauimds -c mauimds_isNodeInitialized | grep -v true|sort | awk -F ':' '{print$2}'|cut -d ' ' -f1)
